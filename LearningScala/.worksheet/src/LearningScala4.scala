...
// go into more depth - but you've got enough to get through this course for now.
	...
	var list = List (1);System.out.println("""list  : List[Int] = """ + $show(list ));$skip(43); 

	for (x <- 2 to 20) {
 		list = x :: list
 		};$skip(22); 
 		println(list);$skip(60); 
 		
 		val listFiltered = list.filter((x: Int) => x%3 == 0);System.out.println("""listFiltered  : List[Int] = """ + $show(listFiltered ));$skip(33); 
 		
 		    println(listFiltered);$skip(35); 
 		
 		var listFiltered2 = List(0);System.out.println("""listFiltered2  : List[Int] = """ + $show(listFiltered2 ));$skip(91); 
 	
 		
 		for (x <- list) {
 		if(x%3 == 0)  {
 			listFiltered2 = x :: listFiltered2
 		}
 		};$skip(32); 
    println(listFiltered2)}
